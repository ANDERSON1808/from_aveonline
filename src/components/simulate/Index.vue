      ...
      <div class=" mx-auto max-w-md shadow-md hover:shadow-lg transition duration-300">
        <div class="py-12 p-10 bg-white rounded-xl">
          <div class="mb-6">
            ...
            <input type="date" @blur="validate('creationDate')" @change="validateDate"
                   :class="validateClass.creationDate" v-model="invoice.creationDate"/>
          </div>
          <div class="">
          ...
          <button
              class="w-full mt-6 text-indigo-50 font-bold bg-red-300 py-3 rounded-md hover:bg-indigo-500 transition duration-300"
              @click="simulate">Simular
          </button>
        </div>
      </div>
    </div>
...
export default {
  setup() {
    ...
    const simulate = () => {
      ...
      let discount = 0;
      let dateSelected = new Date(`${invoice.value.creationDate} 23:59:59`);
      promotions.value.filter(item => {
        let start = new Date(`${item.startDate.substring(0, 10)} 00:00:00`)
        let end = new Date(`${item.startDate.substring(0, 10)} 23:59:59`)
        if (dateSelected.getTime() >= start.getTime() && dateSelected.getTime() <= end.getTime()) {
          discount += item.percentaje
          invoice.value.promotionId = item.id
          return
        }
      })
      invoice.value.totalPay = subtotal - (subtotal * (discount / 100));
      invoice.value.totalPay = invoice.value.totalPay.toFixed(2)
    }
